...
const TYPE_CONFIG: Record<string, { color: string; badge: string }> = {
  ...
  Query: { color: "#3fb950", badge: "badge-green" },
  Procedure: { color: "#ffa657", badge: "badge-orange" },
  Method: { color: "#79c0ff", badge: "badge-blue" },
};
    ...
    >
      ...
      <motion.div variants={itemVariants} className="flex gap-1 border-b border-[#30363d] mb-6 overflow-x-auto">
        {[
          ...
          { id: 'reply', label: 'Assistant Bot', icon: Bot, count: null },
        ].map(tab => (
          <button
            key={tab.id}
                ...
                {(data.reasoning_steps || data.explanation?.map((e: string, i: number) => ({ step: `Step ${i+1}`, icon: 'text', detail: e })) || []).map((step: any, idx: number) => {
                  return (
                    <motion.div
                      ...
                      transition={{ delay: idx * 0.1, ease: [0.22, 1, 0.36, 1] as any }}
              ...
              <div className="p-5 flex items-start gap-4">
                ...
                <div className="flex-1 rounded-lg border border-[#30363d] overflow-hidden">
                  <div className="bg-[#161b22] px-4 py-2 border-b border-[#30363d] flex items-center justify-between">
                    <span className="text-xs font-semibold text-[#c9d1d9]">openissue-bot <span className="font-normal text-[#7d8590]">commented</span></span>
                    {data.is_llm_generated && (
                      <span className="flex items-center gap-1 text-[9px] font-bold text-[#3fb950] bg-[#3fb950]/10 border border-[#3fb950]/20 px-1.5 py-0.5 rounded">
                        <Zap className="w-2.5 h-2.5 fill-[#3fb950]" /> GENERATED BY AI
                      </span>
                    )}
                  </div>
